...
## How to connect
1. Find and install the `homebridge-aranet4-next` plugin in the Homebridge UI or via the command line.
2. Pair your Aranet4 device with your computer via Bluetooth. You can use the `bluetoothctl` command on Linux or the Bluetooth settings on macOS. Then scan for the device and pair it by entering the pin code displayed on the Aranet4 device.
3. You may need to enter a device code generated by Homebridge in the Homebridge logs. This is a one-time step.
4. Now all tree sensors are available in Accessories in the Homebridge UI.
